...
| Host firewall            | UFW handles host ingress; `DOCKER-USER` restricts Dokploy port `3000`; the provider firewall adds an outer layer      | Docker networking and interface changes can invalidate assumptions; verify externally after upgrades and reboots   |
